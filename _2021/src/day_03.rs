...
impl Solution<u64> for Day3 {
    ...
    fn solve_part_two(&self) -> u64 {
        ...
        for v in &self.data {
            if v.iter().enumerate().filter( |(i,&c)| !co2_vals.contains(i) && c == '1' ).count()*2 >= (self.data[0].len() - co2_vals.len()) {
                co2_bit = '1';
            } else {
                co2_bit = '0';
            }
            if v.iter().enumerate().filter( |(i,&c)| !life_vals.contains(i) && c == '1' ).count()*2 < (self.data[0].len() - life_vals.len()) {
                life_bit = '1';
            } else {
                life_bit = '0';
            }
            ...
            if co2_vals.len() + life_vals.len() + 2 == self.data[0].len()*2 {
                let co2_index: usize = *(0..self.data[0].len()).collect::<Vec<_>>().iter().filter( |i| !co2_vals.contains(i) ).collect::<Vec<_>>()[0];
                let life_index: usize = *(0..self.data[0].len()).collect::<Vec<_>>().iter().filter( |i| !life_vals.contains(i) ).collect::<Vec<_>>()[0];
