...
impl Entry {
    pub fn load(input: String) -> Self {
        let filtered_input: String = input.chars().filter( |c| c != &':' ).map( |c| if c == '-' { ' ' } else { c } ).collect();
    ...
    pub fn is_valid_part_one(&self) -> bool {
        let digest: u32 = self.password.chars().map( |c| (String::from(c) == self.c) as u32 ).reduce( |a, b| a + b ).unwrap();
        (digest >= self.low) && (digest <= self.high)
    }
    
    pub fn is_valid_part_two(&self) -> bool {
        let digest: Vec<char> = self.password.chars().enumerate().filter( |t| t.0 as u32 == self.low - 1 || t.0 as u32 == self.high - 1 ).map( |t| t.1 ).collect();
...
impl Solution<u32> for Day2 {
    ...
    fn solve_part_one( &self ) -> u32 {
        self.data.clone().into_iter().map( |e| if e.is_valid_part_one() {1} else {0} ).reduce( |a,b| a + b ).unwrap()
    }
    
    fn solve_part_two( &self ) -> u32 {
        self.data.clone().into_iter().map( |e| if e.is_valid_part_two() {1} else {0} ).reduce( |a,b| a + b ).unwrap()
    }
}
